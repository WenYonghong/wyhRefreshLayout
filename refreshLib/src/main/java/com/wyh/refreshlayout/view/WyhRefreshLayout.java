...
public class WyhRefreshLayout extends FrameLayout {
    private static final String TAG = "WyhRefreshLayout";
    ...
    public WyhRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WyhRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.WyhRefreshLayout);
        mRefreshStyle = ta.getInt(R.styleable.WyhRefreshLayout_refresh_style, STYLE_DEFAULT);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDisenableLoadMore && (sy > ev.getY())) {
                    return false;
                }


                if (Math.abs(ev.getX() - sx) < Math.abs(ev.getY() - sy)) {
                    return interception;
                }
                break;
    ...
    private void move(int moveY) {
        if (mDirection == DIRECTION_DOWN) {
            ...
            if (moveDistance >= mHeadViewHeight) {
                ...
                mIRefreshHead.onPullDown(Math.abs(moveDistance));
            } else {
                mIRefreshHead.onPullDown(Math.abs(moveDistance));
            }
        } else {
